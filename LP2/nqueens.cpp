...
bool solveNQueens(int **board, int col, int n) {
  if (col >= n) { //as we are traversing column by column soo we have reach the limit
    return true;
  }

  for (int i = 0; i < n; i++) { //try every row in that column
    // Check if placing the queen is safe
    if (isSafe(i, col, board, n)) {
      board[i][col] = 1; //if safe place a queen
